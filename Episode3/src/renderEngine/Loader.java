...
public class Loader {
     ...
     */
    public RawModel loadToVAO(float[] positions, int [] indices) { // edit E3
        int vaoID = createVAO();
    ...
    private void bindIndicesBuffer(int [] indices){ // bind the indices into the buffer || E3
        int vboID = GL15.glGenBuffers(); // creating empty vbo
        vbos.add(vboID);
    ...
    private IntBuffer storeDataInIntBuffer(int []data){ // E3
        IntBuffer buffer = BufferUtils.createIntBuffer(data.length);  // creating empty integer list called buffer || parameter is size of the data
        buffer.put(data);
        buffer.flip();
        return buffer;
    }
